...
import torch
from torch.nn import Module, AvgPool2d, Linear,Sigmoid
from models.base_module import Conv_Block, GhostBottleneck


class PFLD_GhostNet_Slim_coco(Module):
    def __init__(self, width_factor=1, input_size=112, landmark_number=98):
        super(PFLD_GhostNet_Slim_coco, self).__init__()
        ...
        print(multi_scale.shape[1])

        self.fc = Linear(multi_scale.shape[1], landmark_number * 2)
        self.fc_visable = Linear(multi_scale.shape[1], landmark_number)
        self.sig = Sigmoid()
    ...
    def forward(self, x):
        ...
        visable = self.fc_visable(multi_scale)
        visable = self.sig(visable)

        return landmarks, visable


if __name__ == "__main__":
    model = PFLD_GhostNet_Slim_coco(input_size=192,landmark_number=17)
    x = torch.ones([1, 3, 256, 192])
    print(model(x))